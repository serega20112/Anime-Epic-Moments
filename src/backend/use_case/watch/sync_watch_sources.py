...
class SyncWatchSourcesUseCase:
    ...
    def __init__(self, anime_api_client: AnimeApiClient, watch_source_sync_service: WatchSourceSyncService):
        self.anime_api_client = anime_api_client
        self.watch_source_sync_service = watch_source_sync_service

    def execute(self, anime_id: int, episode: int, force: bool = False) -> dict[str, int | bool]:
        anime = self.anime_api_client.get_by_id(anime_id)
        if not self.watch_source_sync_service.is_enabled():
            return {"enabled": False, "sources_count": 0}
        ...
        return {"enabled": True, "sources_count": len(sources)}
