...
class SetSavedHighlightUseCase:
    ...
    async def execute(self, highlight_id: int, user_id: int, saved: bool) -> bool:
        result = await self.repo.set_saved(highlight_id=highlight_id, user_id=user_id, saved=saved)
