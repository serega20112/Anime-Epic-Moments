...
from src.backend.use_case.highlight.get_public_top_highlights import GetPublicTopHighlightsUseCase
...
from src.backend.use_case.anime.search_anime_by_description import SearchAnimeByDescriptionUseCase
...
from src.backend.use_case.recommendation.generate_recommendations import GenerateRecommendationsUseCase
from src.backend.use_case.recommendation.refresh_recommendations import RefreshRecommendationsUseCase
...
class Container:
    ...
    def __init__(self):
        ...
        if not Settings.hf_token:
            print("! HF_TOKEN не задан: поиск по описанию работает в fallback-режиме без LLM")
        ...
        self.register_user_use_case = lambda: RegisterUserUseCase(self.user_repository, self.password_service)
        self.login_user_use_case = lambda: LoginUserUseCase(self.user_repository, self.password_service)
        self.logout_user_use_case = lambda: LogoutUserUseCase(self.user_repository)
        self.update_user_profile_use_case = lambda: UpdateUserProfileUseCase(self.user_repository)
        ...
        self.create_highlight_use_case = lambda: CreateHighlightUseCase(self.highlight_repository)
        self.delete_highlight_use_case = lambda: DeleteHighlightUseCase(self.highlight_repository)
        self.edit_highlight_use_case = lambda: EditHighlightUseCase(self.highlight_repository)
        ...
        self.add_favorite_use_case = lambda: AddFavoriteUseCase(self.favorite_repository)
        self.remove_favorite_use_case = lambda: RemoveFavoriteUseCase(self.favorite_repository)
        ...
        self.search_anime_by_description_use_case = lambda: SearchAnimeByDescriptionUseCase(
            self.anime_api_client,
            self.hf_llm_client
        )
        self.autocomplete_anime_use_case = lambda: AutocompleteAnimeUseCase(self.anime_api_client)
        self.get_season_popular_use_case = lambda: GetSeasonPopularUseCase(self.anime_api_client)

        self.generate_recommendations_use_case = lambda: GenerateRecommendationsUseCase(self.recommendation_service)
        self.refresh_recommendations_use_case = lambda: RefreshRecommendationsUseCase(self.recommendation_service)
        ...
        self.upsert_user_anime_status_use_case = lambda: UpsertUserAnimeStatusUseCase(self.watch_repository)
        self.add_watch_source_use_case = lambda: AddWatchSourceUseCase(self.watch_repository)
        self.save_viewing_session_use_case = lambda: SaveViewingSessionUseCase(self.watch_repository)
        self.create_watch_highlight_use_case = lambda: CreateWatchHighlightUseCase(
            CreateHighlightUseCase(self.highlight_repository),
            self.watch_repository
        )
