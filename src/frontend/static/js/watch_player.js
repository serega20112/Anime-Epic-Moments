(function () {
    ...
    const discoverSourcesButton = document.getElementById("discover-sources-button");
    ...
    const allSources = sourcesJson ? JSON.parse(sourcesJson.textContent || "[]") : [];
    ...
    const getSourcesByTranslation = (translationId) => allSources
        .filter((item) => String(item.translation_id) === String(translationId))
        .sort((left, right) => getQualityRank(right.quality_label) - getQualityRank(left.quality_label));
    ...
    const seekTo = (value) => {
        ...
        const nextTime = Math.max(0, Math.min(Number(value || 0), Number(video.duration || 0)));
    ...
    const saveSession = async () => {
        ...
        await fetch(`/watch/${config.animeId}/session`, {
            ...
            body: JSON.stringify({
                ...
                quality_label: qualitySelect ? (qualitySelect.selectedOptions[0]?.textContent || "Auto") : "Auto",
    ...
    const setVideoSource = (sourceId) => {
        ...
        const source = allSources.find((item) => Number(item.source_id) === Number(sourceId));
        if (!source) {
            if (hlsInstance) {
                hlsInstance.destroy();
                hlsInstance = null;
    ...
    const syncQualityOptions = () => {
        ...
        qualitySelect.innerHTML = items.map((item) => `
        ...
        const preferred = items.find((item) => item.quality_label === config.savedQualityLabel) || items[0];
    ...
    highlightEndButton?.addEventListener("click", () => {
        ...
        highlightForm.elements.end_timestamp.value = formatClock(video.currentTime || 0);
        showControls();
    });
    ...
    highlightForm?.addEventListener("submit", async (event) => {
        ...
        const startTimestamp = parseTimestamp(formData.get("start_timestamp") || highlightStart || 0);
        const endTimestamp = parseTimestamp(formData.get("end_timestamp") || (video ? video.currentTime : 0));
    ...
    watchFavoriteButton?.addEventListener("click", async () => {
        if (!window.AEMAnimeUI || typeof window.AEMAnimeUI.addFavorite !== "function") {
            return;
        }
        await window.AEMAnimeUI.addFavorite(watchFavoriteButton.dataset.animeId, watchFavoriteButton);
    });
    ...
    document.querySelectorAll(".status-pill").forEach((button) => {
        button.addEventListener("click", async () => {
            ...
            document.querySelectorAll(".status-pill").forEach((item) => item.classList.remove("is-active"));
    ...
    episodeForm?.addEventListener("change", () => {
        ...
        const episode = episodeField ? Number(episodeField.value || config.episode) : config.episode;
    ...
    if (translationSelect) {
        ...
        const selectedTranslationId = config.selectedTranslationId || Number(translationSelect.value || 0);
