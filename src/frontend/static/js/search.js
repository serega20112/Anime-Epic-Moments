(function () {
    ...
    const renderResults = (items) => {
        ...
        if (window.AEMAnimeUI && typeof window.AEMAnimeUI.renderAnimeCards === "function") {
    ...
    form.addEventListener("submit", async (event) => {
        ...
        if (ageRating === "18+") {
            const confirmed = window.confirm("Показать 18+ контент? Подтвердите, что вам есть 18 лет.");
            if (!confirmed) {
                if (resultsMeta) {
                    resultsMeta.textContent = "Показ 18+ отменён";
                }
                results.innerHTML = "<p>Поиск 18+ отменён. Выберите другой возрастной рейтинг.</p>";
                return;
            }
            adultConfirmed = true;
        }
        ...
        let response = await fetch(`/anime/api/search/description?${params.toString()}`);
        if (!response.ok) {
        ...
        if (!Array.isArray(payload) && payload.requires_age_confirmation) {
            const confirmed = window.confirm(payload.message || "Подтвердите, что вам есть 18 лет.");
            if (!confirmed) {
                if (resultsMeta) {
                    resultsMeta.textContent = "Показ 18+ отменён";
                }
                results.innerHTML = "<p>Контент 18+ скрыт до подтверждения возраста.</p>";
                return;
            }
            params = buildParamsFromForm(formData, true);
            response = await fetch(`/anime/api/search/description?${params.toString()}`);
            if (!response.ok) {
                results.innerHTML = "<p>Ошибка поиска. Попробуйте позже.</p>";
                if (resultsMeta) {
                    resultsMeta.textContent = "Не удалось получить результаты";
