...
from backend.application.use_cases.auth.login_user import InvalidCredentialsError
from backend.presentation.api.v1 import auth_route as auth_route_module
from backend.presentation.api.v1.auth_route import auth_bp
from backend.presentation.api.v1.index_route import index_bp
...
def test_refresh_session_rotates_tokens(flask_app_factory, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        "container",
        SimpleNamespace(
            token_blocklist=SimpleNamespace(is_revoked=lambda token: False, revoke=lambda token, ttl: None)),
    )
