...
async def test_rate_limiter_blocks_after_limit(attempts, limit, expected_allowed):
    ...
    for _ in range(attempts):
        decision = await limiter.hit("login", "127.0.0.1", limit=limit, window_seconds=60)
...
@pytest.mark.unit
async def test_rate_limiter_tracks_retry_after():
    """Проверяем, что RateLimiter возвращает retry_after для заблокированного ключа."""
    limiter = RateLimiter(KeyValueStore(redis_url=None, namespace="test"))
    await limiter.hit("search", "user", limit=1, window_seconds=60)

    decision = await limiter.hit("search", "user", limit=1, window_seconds=60)
