...
class TestRewriteHlsManifest:
    ...
    def test_rewrites_wrapped_citation_uri(self):
        manifest = '#EXT-X-KEY:METHOD=AES-128,URI="key.bin"'
        base = "https://cdn.libria.fun/master.m3u8"
        result = rewrite_hls_manifest(manifest, base, _proxy)
        assert 'URI="' + _proxy("https://cdn.libria.fun/key.bin") + '"' in result

    def test_preserves_empty_lines_and_comments(self):
        manifest = "#EXTM3U\n\n#EXT-X-VERSION:3\n"
        result = rewrite_hls_manifest(manifest, "https://cdn.libria.fun/m.m3u8", _proxy)
        assert result == "#EXTM3U\n\n#EXT-X-VERSION:3"

    def test_keeps_absolute_segments_unchanged_shape(self):
